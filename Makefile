...
configure_norestart = \
  $(call header,CONFIGURE $(1) ($(RUNTIME_LOG_DIR)) => $(RUNTIME_BIN) $(2)); \
...
configure = $(call configure_norestart,$(1)) && $(restart_docker)
...
dev: $(RUNTIME_BIN) ## Installs a set of local runtimes. Requires sudo.
	@$(call configure_norestart,$(RUNTIME),--net-raw)
	@$(call configure_norestart,$(RUNTIME)-d,--net-raw --debug --strace --log-packets)
	@$(call configure_norestart,$(RUNTIME)-p,--net-raw --profile)
	@$(call configure_norestart,$(RUNTIME)-vfs2-d,--net-raw --debug --strace --log-packets --vfs2)
	@$(call restart_docker)
.PHONY: dev
...
docker-tests: load-basic-images $(RUNTIME_BIN)
...
packetdrill-tests: load-packetdrill
	@$(call install_runtime) # Clear flags.
	@$(call test_runtime,$$($(call query,attr(tags, packetdrill, tests(//...)))))
...
packetimpact-tests: load-packetimpact $(RUNTIME_BIN)
	...
	@sudo modprobe ip6table_filter
	@$(call install_runtime) # Clear flags.
	@$(call test_runtime,--jobs=HOST_CPUS*3 --local_test_jobs=HOST_CPUS*3 $$($(call query,attr(tags, packetimpact, tests(//...)))))
...
containerd-test-%: load-basic_alpine load-basic_python load-basic_busybox load-basic_resolv load-basic_httpd load-basic_ubuntu $(RUNTIME_BIN)
	...
	@$(call sudo,test/root:root_test,--runtime=root -test.v)
