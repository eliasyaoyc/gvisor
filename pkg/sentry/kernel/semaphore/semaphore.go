...
func (r *Registry) semctlStat(semusz, semaem uint32) *linux.SemInfo {
...
func (r *Registry) IPCInfo() *linux.SemInfo {
	// IPC_INFO doesn't support SemUsz.
	return r.semctlStat(0, linux.SEMAEM)
}

// SemInfo Return a seminfo structure containing the same information as
...
func (r *Registry) SemInfo() *linux.SemInfo {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.semctlStat(uint32(len(r.semaphores)), uint32(r.totalSems()))
}
