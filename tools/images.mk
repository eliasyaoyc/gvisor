...
%-all-images: $(patsubst %,$*-%,$(ALL_IMAGES))
load-%-images: ## Load a specified directory, e.g. load-basic-images.
load-%-images: $(patsubst %,load-%,$(subst /,_,$(subst images/,,$(shell find images/$* -name Dockerfile -exec dirname {} \;))))
...
loaded0_$(1)=load-$$(1): # Already available.
...
pull = docker pull $(DOCKER_PLATFORM_ARGS) $(call remote_image,$(1)):$(call tag,$(1))
pull-%: ## Force a repull of the image.
	@$(call pull,$*)
...
rebuild = \
  ...
  docker build $(DOCKER_PLATFORM_ARGS) \
    ...
    -t "$(call remote_image,$(1)):$(call tag,$*)" \
    $$T && \
  rm -rf $$T)
rebuild-%: ## Force rebuild an image locally.
	@$(call rebuild,$*)
...
load-%: ## Pull or build an image locally.
	@$(call pull,$*) || $(call rebuild,$*)
	@docker tag $(call remote_image,$*):$(call tag,$*) $(call local_image,$*):$(call tag,$*)
